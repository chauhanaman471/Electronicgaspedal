/*******************************************************************************
* File Name: JOYSTICK_ADC_XY_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_Start
********************************************************************************
*
* Summary:
...
void JOYSTICK_ADC_XY_IntClock_Start(void) 
{
    ...
    JOYSTICK_ADC_XY_IntClock_CLKEN |= JOYSTICK_ADC_XY_IntClock_CLKEN_MASK;
	JOYSTICK_ADC_XY_IntClock_CLKSTBY |= JOYSTICK_ADC_XY_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_Stop
********************************************************************************
*
* Summary:
...
void JOYSTICK_ADC_XY_IntClock_Stop(void) 
{
    ...
    JOYSTICK_ADC_XY_IntClock_CLKEN &= (uint8)(~JOYSTICK_ADC_XY_IntClock_CLKEN_MASK);
	JOYSTICK_ADC_XY_IntClock_CLKSTBY &= (uint8)(~JOYSTICK_ADC_XY_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((JOYSTICK_ADC_XY_IntClock_CLKEN & JOYSTICK_ADC_XY_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = JOYSTICK_ADC_XY_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = JOYSTICK_ADC_XY_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* JOYSTICK_ADC_XY_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(JOYSTICK_ADC_XY_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(JOYSTICK_ADC_XY_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = JOYSTICK_ADC_XY_IntClock_GetSourceRegister();
    uint16 oldDivider = JOYSTICK_ADC_XY_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = JOYSTICK_ADC_XY_IntClock_CLKEN & JOYSTICK_ADC_XY_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(JOYSTICK_ADC_XY_IntClock_DIV_PTR, clkDivider);
                JOYSTICK_ADC_XY_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                JOYSTICK_ADC_XY_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(JOYSTICK_ADC_XY_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((JOYSTICK_ADC_XY_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(JOYSTICK_ADC_XY_IntClock_DIV_PTR, clkDivider);
				JOYSTICK_ADC_XY_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(JOYSTICK_ADC_XY_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    JOYSTICK_ADC_XY_IntClock_MOD_SRC |= modeBitMask & (uint8)JOYSTICK_ADC_XY_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    JOYSTICK_ADC_XY_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(JOYSTICK_ADC_XY_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return JOYSTICK_ADC_XY_IntClock_MOD_SRC & (uint8)(JOYSTICK_ADC_XY_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = JOYSTICK_ADC_XY_IntClock_GetDividerRegister();
    uint8 oldSrc = JOYSTICK_ADC_XY_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        JOYSTICK_ADC_XY_IntClock_MOD_SRC |= CYCLK_SSS;
        JOYSTICK_ADC_XY_IntClock_MOD_SRC =
            (JOYSTICK_ADC_XY_IntClock_MOD_SRC & (uint8)(~JOYSTICK_ADC_XY_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return JOYSTICK_ADC_XY_IntClock_MOD_SRC & JOYSTICK_ADC_XY_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    JOYSTICK_ADC_XY_IntClock_PHASE = clkPhase & JOYSTICK_ADC_XY_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: JOYSTICK_ADC_XY_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return JOYSTICK_ADC_XY_IntClock_PHASE & JOYSTICK_ADC_XY_IntClock_PHASE_MASK;
}
